...
alg_name_list = ['CP-1', 'CP-2', 'CP-3', 'CP-4', 'NA-all', 'NA-sub', 'Random']
...
for session_size, session_count in zip(session_size_list, session_count_list):
	...
	for i, alg_name in enumerate(alg_name_list):		
		...
		axes[i].set_ylabel('Accumulated number of clients', fontsize=18)
	...
	plt.suptitle('Distribution of regions of chosen datacenters with session size = ' + session_size, fontsize=20, y=1.05);
	plt.savefig('ClientAssignment_for_SessionSize-' + session_size + '.pdf', bbox_inches='tight')